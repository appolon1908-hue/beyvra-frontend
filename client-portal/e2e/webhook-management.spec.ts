...
test("verified users can manage webhook integrations through the current API contract", async ({ request, baseURL }) => {
  ...
  const create = await request.post(`${origin}/api/notification/webhooks/`, { data: { url: "https://example.com/codestra-webhook", secret: "webhook-signing-secret-2026", categories: ["SECURITY"] } });
  expect(create.ok(), await create.text()).toBeTruthy();
  const webhook = await create.json();
  const testDelivery = await request.post(`${origin}/api/notification/webhooks/${webhook.id}/test/`, { data: {} });
  expect(testDelivery.ok(), await testDelivery.text()).toBeTruthy();
  const deliveries = await request.get(`${origin}/api/notification/webhooks/${webhook.id}/deliveries/`);
  expect(deliveries.ok()).toBeTruthy();
  const update = await request.patch(`${origin}/api/notification/webhooks/${webhook.id}/`, { data: { is_active: false } });
  expect(update.ok()).toBeTruthy();
  const remove = await request.delete(`${origin}/api/notification/webhooks/${webhook.id}/`);
