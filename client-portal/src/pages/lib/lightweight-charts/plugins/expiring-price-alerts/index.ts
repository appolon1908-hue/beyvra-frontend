...
import { sampleAlertLineData } from "../../data/dummy";



export function expiringPriceAlerts(chart: IChartApi, data?: LineData<Time>[]) {
  const lineSeries = chart.addLineSeries({
    lastPriceAnimation: LastPriceAnimationMode.OnDataUpdate,
  });
  const customData = data ?? sampleAlertLineData(); // 622 items
  const [initialData, realtimeUpdates] = [customData.slice(0, 400), customData.slice(400)];
