...
import { fixedTimeDefaults } from "api/demo/fixedTimeDefaults";
...
const PlatformChartContainer: React.FunctionComponent<PlatformProps> = ({ themeSelect }) => {
  ...
  const demoConfig = workspaceBootstrap?.rules ?? fixedTimeDefaults;
  const isTicketOpen = overlay.type === "trade";
  const [amount, setAmount] = useState(fixedTimeDefaults.minAmount * 100);
