...
export function TradeMarkerSummary({ markers, serverNow }: { markers: TradeChartMarker[]; serverNow: number }) {
  return <div className="trade-marker-summary" role="group" aria-label="Demo trade chart markers">{markers.slice(0, 4).map((marker) => {
...
export function TradeTicket({ symbol, quote, amount, setAmount, duration, setDuration, orderState, orderError, connectionState, submitDemoOrder, trades, close, open, durations, minAmount, maxAmount, amountStep, payoutRate }: {
  ...
  return <div id="platform-order-ticket" className={`trade-ticket-shell${open ? " is-open" : ""}`} data-open={open}><div className="trade-ticket-header"><span>Demo order</span><button type="button" onClick={close} aria-label="Close demo trade ticket">×</button></div><div className="demo-order-form" aria-label="Demo order ticket">
    ...
    <p className="demo-estimate">Simulation preview includes estimated fees, available virtual balance, and risk checks. No real order or money movement occurs.</p>{!canSubmit && <p className="demo-safety-state" role="status">Waiting for a trusted market quote before enabling demo orders.</p>}<div className="demo-direction-actions"><button type="button" className="demo-up" disabled={!canSubmit} onClick={() => submitDemoOrder("up")}>Buy (simulated)</button><button type="button" className="demo-down" disabled={!canSubmit} onClick={() => submitDemoOrder("down")}>Sell (simulated)</button></div><p className="demo-quote">Quote: {quote ? quote.toFixed(2) : "Unavailable"}</p><p role="status" aria-live="polite">{orderState === "submitting" ? "Previewing and submitting…" : orderState === "accepted" ? "Simulated order accepted." : orderError}</p><button type="button" className="demo-how-it-works">ⓘ How it works</button><OpenTrades trades={trades} />
  </div></div>;
}
