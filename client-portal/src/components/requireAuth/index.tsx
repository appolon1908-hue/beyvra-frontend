...
/**
 * Safely extracts JWT payload without trusting it for security decisions.
 * @param token JWT token string
 * @returns Decoded payload or null if invalid
 * @warning Do NOT use this for authorization - only for display purposes
 */
...
const RequireAuth = () => {
  ...
  useEffect(() => {
    ...
    // Session state validation is done by backend - we trust the response
  ...
  const { mutate } = useRefreshToken({
    onSuccess: () => {
      ...
      console.error("Error refreshing the token", { errorType: typeof error });
  ...
  const resetTimer = () => {
    try {
      ...
      logInternalError(error, { endpoint: "app.resetIdleTimer" });
    }
  };
  ...
  useEffect(() => {
    ...
    // Only fetch KYC for actual users, not guest demos (determined by backend)
    if (bootstrap === "USER_READY") {
      mutateKYC({ token: cookies.access_token });
    }
  ...
  }, [cookies.access_token, mutateKYC, bootstrap]);
  ...
  useEffect(() => {
    if (cookies?.access_token && cookies.refresh_token) {
      ...
      const tokenPayload = tryDecodeJwtPayload(cookies.access_token);
      if (!tokenPayload || typeof tokenPayload.exp !== 'number') {
        logInternalError(new Error("Invalid token payload"), { endpoint: "auth.token_decode" });
        return;
      }

      const tokenExpirationTime = new Date(tokenPayload.exp * 1000);
      const currentTime = new Date();
      const timeUntilExpiration = tokenExpirationTime.getTime() - currentTime.getTime();
